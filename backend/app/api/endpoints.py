...
def get_ledger(
    ...
    # Fetch active accounts and their starting balances
    active_accounts = db.query(Account).filter(Account.entity.in_(entities), Account.is_on_budget == True).all()
    account_ids = [a.id for a in active_accounts]
    total_starting_balance = sum(a.starting_balance for a in active_accounts)

    balance_calc = (func.sum(LedgerEntry.amount).over(
        order_by=(LedgerEntry.date, LedgerEntry.id)
    ) + total_starting_balance).label('running_balance')

    query = (db.query(LedgerEntry, Account.entity, balance_calc)
             .join(Account, LedgerEntry.account_id == Account.id)
             .filter(LedgerEntry.account_id.in_(account_ids))
             .order_by(LedgerEntry.date, LedgerEntry.id).all())
...
def delete_rule(rule_id: UUID, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    db.delete(rule) # This will cascade delete future projections
    db.commit()
    return {"status": "deleted"}

@router.delete("/transactions/{tx_id}")
def delete_transaction(tx_id: UUID, db: Session = Depends(get_db)):
    tx = db.query(LedgerEntry).filter(LedgerEntry.id == tx_id).first()
    if not tx:
...
def get_accounts(entities: List[EntityType] = Query([...]), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return db.query(Account).filter(Account.entity.in_(entities)).all()
...
def update_account(account_id: UUID, account_data: AccountCreate, db: Session = Depends(get_db)):
...
def delete_account(account_id: UUID, db: Session = Depends(get_db)):
    ...
    db.query(LedgerEntry).filter(LedgerEntry.account_id == account_id).delete()
    db.delete(db_account)
    db.commit()
    return {"status": "deleted"}

@router.post("/rules", response_model=RecurringRuleOut)
def create_rule(rule_data: RecurringRuleCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    db_rule = RecurringRule(**rule_data.model_dump())
    db.add(db_rule)
    db.commit()
    db.refresh(db_rule)
    
    # Generate new projected ledger entries
...
def update_rule(rule_id: UUID, rule_data: RecurringRuleCreate, db: Session = Depends(get_db)):
    ...
    # Wipe future projections for this rule
...
def create_transaction(tx_data: LedgerEntryCreate, db: Session = Depends(get_db)):
    db_tx = LedgerEntry(**tx_data.model_dump(), status=LedgerStatus.ACTUAL)
    db.add(db_tx)
    db.commit()
    db.refresh(db_tx)
    return db_tx
...
def match_transaction(ghost_id: UUID, match_data: MatchTransactionRequest, db: Session = Depends(get_db)):
    ghost = db.query(LedgerEntry).filter(LedgerEntry.id == ghost_id).first()
    if not ghost:
    ...
    # If this projection is linked to an asset (e.g. a loan repayment)
    # Update the current principal value of the asset
    if ghost.asset_id:
        asset = db.query(Asset).filter(Asset.id == ghost.asset_id).first()
        if asset:
            # For a loan, a negative ledger amount (payment) reduces the debt principal
            # Debt is current_value. 
            payment_amount = abs(match_data.actual_amount)
            asset.current_value = max(0, asset.current_value - payment_amount)
            
            # Record in history
            history = AssetValueHistory(asset_id=asset.id, date=match_data.actual_date, value=asset.current_value)
            db.add(history)
...
@router.get("/stats")
def get_stats(entities: List[EntityType] = Query([...]), base_currency: str = Query("AUD"), db: Session = Depends(get_db)):
    accounts = db.query(Account).filter(Account.entity.in_(entities)).all()
    ...
    for acc in accounts:
        ...
        # Current balance = starting_balance + sum of all ACTUAL entries for this account
        actual_sum = db.query(func.sum(LedgerEntry.amount)).filter(
...
def get_assets(entities: List[EntityType] = Query([...]), db: Session = Depends(get_db)):
    assets = db.query(Asset).filter(Asset.entity.in_(entities)).all()
    result = []
    for a in assets:
        item = AssetOut.model_validate(a)
        # Compute equity if this asset has a linked loan
        if a.linked_loan_id:
            loan = db.query(Asset).filter(Asset.id == a.linked_loan_id).first()
            if loan:
                item.equity = a.current_value - loan.current_value
                item.lvr = round((loan.current_value / a.current_value) * 100, 1) if a.current_value > 0 else None
        result.append(item)
    return result
...
def create_asset(asset_data: AssetCreate, db: Session = Depends(get_db)):
    ...
    history = AssetValueHistory(asset_id=db_asset.id, date=datetime.date.today(), value=db_asset.current_value)
    db.add(history)
    db.commit()
...
def update_asset(asset_id: UUID, asset_data: AssetCreate, db: Session = Depends(get_db)):
...
def update_asset_value(asset_id: UUID, update: AssetValueUpdate, db: Session = Depends(get_db)):
...
@router.get("/v2/stats")
def get_stats_v2(entities: List[EntityType] = Query([...]), base_currency: str = Query("AUD"), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    budget_stats = get_stats(entities, base_currency, db)
    
    # We don't need the full forecast for the basic net worth stats
    # which are "Current", but we'll keep the placeholders for future extension
    all_ledger = [] # Reserved for future time-series amortization data
    ...
    for a in assets:
        if a.is_liability:
            ...
            # For purely informational purposes, compute equity if linked
            if a.linked_loan_id:
                loan = db.query(Asset).filter(Asset.id == a.linked_loan_id).first()
                if loan:
...
def sync_stock_prices(db: Session = Depends(get_db)):
    """Refresh all assets that have a ticker symbol."""
    assets = db.query(Asset).filter(Asset.ticker != None).all()
    updated_count = 0
    ...
    for asset in assets:
        ...
        if new_price is not None:
            ...
            history = AssetValueHistory(asset_id=asset.id, date=datetime.date.today(), value=new_price)
            db.add(history)
            updated_count += 1
...
def get_asset_projection(asset_id: UUID, db: Session = Depends(get_db)):
    asset = db.query(Asset).filter(Asset.id == asset_id).first()
    if not asset:
        raise HTTPException(status_code=404, detail="Asset not found")
        
    # Get forecast for the entities this asset belongs to
    dummy_req = type('obj', (object,), {'entities': [asset.entity], 'horizon_days': 365})
    ledger = generate_forecast(dummy_req, db)
    
    projection = calculate_projected_loan_balances(asset, ledger)
    return projection
